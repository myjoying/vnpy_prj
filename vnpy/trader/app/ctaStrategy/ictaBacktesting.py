...
########################################################################
class CtBacktestingEngine(object):
    """
    CTA回测引擎
    函数接口和策略引擎保持一样，
    从而实现同一套代码从回测到实盘。
    ...
    def __init__(self):
        ...
        self.dbClient = pymongo.MongoClient(globalSetting['mongoHost'], globalSetting['mongoPort'])        # 数据库客户端
        self.dbCursor = {}        # 数据库指针
    ...
    #----------------------------------------------------------------------
    def setDatabase(self, dbName, symbol, isbase = False, dbkey='Default'):
        """设置历史数据所用的数据库"""
        self.dbName[dbkey] = dbName
        if isbase:
            self.basedbName = dbkey
        self.symbol = symbol
    ...
    def loadHistoryData(self):
        """载入历史数据"""
        collection = self.dbClient[self.dbName[self.basedbName]][self.symbol]          
        ...
        self.dbCursor[self.basedbName] = collection.find(flt).sort('datetime')
        
        self.output(u'载入完成，数据量：%s' %(initCursor.count() + self.dbCursor[self.basedbName].count()))
    ...
    def runBacktesting(self):
        ...
        count = 0;
        for d in self.dbCursor[self.basedbName]:
            data = dataClass()
            data.__dict__ = d
            count = count + 1
            if(count % 100 == 0):
                print u"data items = %d",(count)
            func(data)     
    ...
    #----------------------------------------------------------------------
    def loadDataRange(self, dbKey, start_time, end_time, forward_days = 0):
        '''
        加载选定日期范围[start_time-forward_days, end_time]的数据
        '''
        collection = self.dbClient[self.dbName[dbKey]][self.symbol] 
    ...
    #----------------------------------------------------------------------
    def insertCBData(self, dbName, collectionName, data):
        if self.dbClient:
            d = data.__dict__
            conditon = {'_id': d['_id']}
            db = self.dbClient[dbName]
            collection = db[collectionName]
            collection.update_one(conditon, {'$set':d}, True)
            
    #----------------------------------------------------------------------
    def removeCBData(self, dbName, collectionName, key):
        if self.dbClient:
            flt = {key:{'$gte':self.dataStartDate}}
            db = self.dbClient[dbName]
            collection = db[collectionName]
    ...
    def showBacktestingResult(self):
        ...
        for result in d['resultList']:
            print u'交易编号 %d\t | 时间: %s - %s| 价格：%s - %s | 盈利:%s'%(count, str(result.entryDt), str(result.exitDt), \
                        formatNumber(result.entryPrice), formatNumber(result.exitPrice),formatNumber((result.exitPrice-result.entryPrice)/result.entryPrice))
            count += 1
