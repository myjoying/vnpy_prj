...
class CtaEngine(object):
    ...
    #----------------------------------------------------------------------
    def registerSpeEvent(self, event_type, processfunc):
        self.eventEngine.register(event_type, processfunc)
    ...
    #----------------------------------------------------------------------
    def updateData(self, dbName, collectionName, data):
        """插入数据到数据库（这里的data可以是VtTickData或者VtBarData）"""
        self.mainEngine.dbUpdate(dbName, collectionName, data.__dict__, {'_id': data.__dict__['_id']}, True) 
        
    #----------------------------------------------------------------------
    def deleteData(self, dbName, collectionName, start_time=None):
        """删除数据"""
        if start_time!=None:
            flt = {'datetime':{'$gte':start_time}}
        else:
            flt = {}
        self.mainEngine.dbDelete(dbName, collectionName, flt)       
        
    #----------------------------------------------------------------------
    def queryData(self, dbName, collectionName, d, sortKey='', sortDirection=ASCENDING):
        """插入数据到数据库（这里的data可以是VtTickData或者VtBarData）"""
        return self.mainEngine.dbQuery(dbName, collectionName, d, sortKey, sortDirection)   
    ...
    def loadStrategy(self, setting):
        ...
        #载入回测日期设置
        self.dataStartDate = datetime.strptime(setting['start'], '%Y%m%d')       # 回测数据开始日期，datetime对象
        self.dataEndDate = datetime.strptime(setting['end'], '%Y%m%d')         # 回测数据结束日期，datetime对象
        initTimeDelta = timedelta(self.initDays)
        self.strategyStartDate = self.dataStartDate + initTimeDelta   # 策略启动日期（即前面的数据用于初始化），datetime对象        
    ...
    def runBacktesting(self):
        ...
        self.writeCtaLog(u'开始回测')
        
        for strategy_name,strategy in self.strategyDict.items():
            ...
            dbCursor = self.mainEngine.dbQuery(self.basedbName, strategy.vtSymbol, flt, sortKey='datetime')

            
            self.writeCtaLog(u'载入完成，数据量：%s' % np.size(dbCursor))
            ...
            strategy.trading = True
            strategy.onStart()
            self.writeCtaLog(u'策略启动完成')
            
            self.writeCtaLog(u'开始回放数据')
